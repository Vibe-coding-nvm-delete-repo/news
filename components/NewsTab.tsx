  ...
  const generateReport = async () => {
    ...
    const searchKeyword = async (
      ...
      try {
        ...
        markStepStart('read-response', 'Decoding OpenRouter JSON payload.');
        let data: any;
        try {
          ...
          markStepError(
            'read-response',
            `Failed to parse JSON response: ${readError.message}`
          );
          throw new Error(
            `Unable to decode OpenRouter response JSON: ${readError.message}`
          );
        }
